...
- 内置法师/死亡骑士专精规则；通常合并 JustAC 当前 Burst Trigger 配置与新版法术 ID，
  12.1 奥法和冰 DK 改用各自精确保留集
- 火法不伪造读条末尾 150ms 的燃烧/火冲双法术编织；主循环只做空闲帧可证明决策，
  读条期间继续严格保护不打断
- 冰法按成功施法事件推进开场序列，冰霜射线始终完整引导；无法取得射线跳数、
  冰刺数或 Rapid Refreezing 状态时回退 JustAC
- 冰/邪 DK 默认由 JustAC 决策；保留的实验性自有源实现资源、Proc、目标数和可见增益
  均完整的优先级切片，不可证明的状态回退 JustAC，且只在玩家显式选择时启用
- DK 按住 M4 即表示正在跑机制或清理尾怪：仍按 JustAC 原队列使用安全普通填充，
  同时保留爆发、饰品/药水；冰 DK 精确保留冰柱、冰霜巨龙之怒，以及已学会的吐息和
  死神印记，不再保留符文武器增效与亡者复生，冷酷严冬仍正常释放；
  12.1 邪 DK 精确保留亡者大军和黑暗突变，腐化与灵魂收割按 JustAC 普通循环释放
- DK 三系的 M4/M5 都跳过死亡之握；抓怪和敌人位移完全保留给玩家手动操作
- 12.1 冰/邪 DK 不把“按住时必须有技能可发”当作规则：原推荐队列过滤完后允许空动作，
  不因移动或队列为空自行注入凛风冲击、传染或凋零缠绕
- 冰 DK 仅在最近 10 秒内成功施放冰霜之柱（或可明确观测到其当前光环）后才放行
  冰霜巨龙之怒；旧的冰霜之柱施法记录不会跨整个冷却周期继续生效；
  JustAC 提前推荐冰霜巨龙之怒时会跳过并继续读取后续队列；按钮明确变为召回法术
  `1265384` 后，二次释放完全交回 JustAC
- 冰 DK 跑机制使用 M4 时只过滤 JustAC 的真实队列：队列提示凛风冲击就执行；没有
  提示便不从 Proc、高亮或专精兜底额外注入远程技能，必要时保持 M4 空输出；M5 在
  目标被可靠确认超过 5 码时只接受 JustAC 队列中的凛风冲击，没有提示就暂停其他动作
  与兜底并继续跑路
- 冰 DK M5 首次施放冰霜巨龙之怒后固定优先接亡者复生；亡者复生处于冷却、状态未知、
  未学习或未绑定时立即放弃跟随并继续 JustAC，不会卡住，二次召回不重复触发
- 奥术宝珠依赖人物面向且无法在副本内可靠自动瞄准：12.1 奥法的 M4/M5 移动时
  都跳过；普通停止移动后两路连续静止满 0.8 秒恢复。成功施放闪现或
  闪光术后，两路都从服务器确认事件起等待 2 秒再恢复宝珠
- 奥术宝珠的可用性使用 JustAC 的多充能本地追踪，不再把 `1/2` 时正在恢复的第一层
  误判成整项冷却；剩余一层仍可按自有优先级使用，`0/2` 才视为不可用，状态未知则
  原样回退 JustAC
...
- 12.1 奥法真实移动时不会因奥冲、飞弹或宝珠被安全过滤，就把后排奥术弹幕当成
  瞬发填充；队列第 2 位及以后的弹幕必须由 `arcane121` 使用正常 APL 谓词正面证明，
  false/unknown/接口缺失/抛错均跳过并允许空动作。弹幕原本就在队首时保持源的决定，
  因而移动本身绝不会降低奥术齐射、充能或爆发窗口门槛
- 若战斗中节能施法光环不可读，但浮光掠影已学习、飞弹已学习且当前可用，移动中的
  实际飞弹推荐允许每次连续移动试放一次；首次失败后锁到真实停步，成功则正常保护
  完整引导。该试放只注册到 12.1 奥法飞弹，不扩散到其他读条或引导技能
- 试放动作导出期间收到停步事件，会取消该动作实例并完整导出一帧下一安全技能；之后
  若新一轮静止推荐仍是奥术飞弹，则按新的正常推荐放行，不继承刚取消的移动试放状态；
  速度数据为 secret 时，STOP 的 250ms 防抖确认期内持续禁止重新建立试放
- 12.1 奥法遇到不可读取的光环或动态条件时不猜，并原样保留 JustAC 剩余动作的相对
  顺序；不会再把奥冲插入或移动到 JustAC 推荐的弹幕之前。Touch 同目标凭据、M4 保留、
  移动/引导和技能合法性等安全过滤仍可删除不合法动作，但不得借回退重写优先级
...
- 12.1 奥术涌动、疾咒师每场首个奥术宝珠、涌动窗内符合条件的大法师之触，以及
  ...
  不把部分可观测规则冒充完整 APL
- 大法师之触在 M5 输出中携带逐动作 `offGCD` 标记：自有源按涌动配对或直接触规则
  选中后，Windows/macOS 只为该动作绕过 `queueReady`，仍保留读条、引导、归属、
  可用性和绑定保护；M4 的独立门控不受影响
- 12.1 M5 使用明确的可靠性配对：涌动和触都就绪就立即先涌动，服务器确认涌动成功后
  下一动作直接优先触；涌动被正面证明未学习、未绑定、不可用或在冷却时，触就绪便
  直接释放，不要求上一弹幕/Bolt，也不要求剩余冷却阈值
- 涌动→触凭据绑定当前敌对目标 GUID；切换目标、死亡/不可攻击、触失败/中断、意外
  GCD 或 10 秒超时都会取消，切回旧目标不会恢复已经清除的凭据。M4 始终保留两技能
- 日怒四充能且奥术齐射达到精确上限 25 层时，在涌动明确结束或剩余时间被证明大于
  最大 GCD 的条件下，无需节能施法即可由自有源选择奥术弹幕
- 日怒通过当前装备物品公开的奥法套装奖励法术 ID 精确区分 S2 两件套与四件套：
  奥术之魂未激活时，两件套直接释放已触发的棱彩飞弹；四件套第一条棱彩飞弹分支
  要求累计能量达到 8 层，明确不足时继续判断弹幕。装备或层数不可读时保持 JustAC
  原队列，不用“光环暂时不存在”猜测套装件数
...
- 移动事件具备战斗内防抖；实际施放连续失败时自动换下一项；仅显式登记历史兼容兜底的
  专精才继续尝试该兜底，否则允许输出为空
...
- 枯萎凋零/亵渎真正放置成功后仍按 10 秒场地时间抑制重复推荐；每次成功施放记入
  固定 30 秒顺序充能计时，不依赖大秘境战斗中的 secret 冷却值。两层都消耗时会在
  第 30 秒提示“枯萎凋零1”，再过 30 秒提示“枯萎凋零2”
- 枯萎凋零的中央文字和 TTS 都使用简短充能提示：“枯萎凋零1”或“枯萎凋零2”；
  主动饰品同样只显示并播报“饰品1”或“饰品2”。
...
逐函数调用顺序、三态谓词、M5/M4 扫描器、法师三系自有源状态机以及像素/Windows
发送路径见 [`JustACBridge.core/BRIDGE_CODE_LOGIC.md`](JustACBridge.core/BRIDGE_CODE_LOGIC.md)。
