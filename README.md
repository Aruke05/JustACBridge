...
- M5 主推荐先处理策略明确登记且确实缺失的自身维护 Buff，再在静止时执行 JustAC
  第一推荐；仅跳过策略明确登记的非循环工具，移动时还会跳过不可移动读条、蓄力
  和引导，选择队列中首个安全替代
...
- 12.1 奥法移动时精确识别浮光掠影＋节能施法的奥术飞弹，以及气定神闲下的瞬发
  奥术冲击；天赋或玩家 Buff 无法确认时保守跳过，不靠按钮发光猜测
- Windows M5 无损位收到魔爆术 `1449` 时先观察 `100ms`；推荐未变才释放，期间切换
  到其他技能则立即采用新推荐
- 奥法棱彩屏障、冰法寒冰护体在自身对应 Buff 明确不存在、当前至少有 2 层充能且
  快捷键已绑定时，优先加入 M5/M4；自动循环只花额外充能，始终保留 1 层供手动释放
