...
- 12.1 奥法遇到不可读取的光环层数时不猜：只要回退 JustAC 的队列含未被证明的弹幕，
  M4/M5 都把已知基础填充奥冲放到它之前；四项队列漏掉奥冲时仅补入该奥冲。静止
  优先奥冲，移动时奥冲被安全过滤后仍可使用弹幕；明确证明的弹幕分支不受影响
...
- 12.1 奥术涌动、疾咒师每场首个奥术宝珠、涌动窗内符合条件的大法师之触，以及
  ...
  不把部分可观测规则冒充完整 APL
- 大法师之触在 M5 输出中携带逐动作 `offGCD` 标记：弹幕/棱彩箭成功事件令自有源
  选中触后，Windows/macOS 只为该动作绕过 `queueReady`，仍保留读条、引导、归属、
  可用性和绑定保护；M4 的独立门控不受影响
- 12.1 大爆发在服务器确认涌动成功后进入短状态机，严格按
  `奥术飞弹 → 奥术弹幕 → 大法师之触` 推进；每一步只接受
  `UNIT_SPELLCAST_SUCCEEDED`，目标改变、死亡/不可攻击、失败/中断、意外 GCD 或 10 秒
  总超时都会取消并立即回到普通优先级
- 涌动/触/弹幕的爆发凭据绑定当前敌对目标 GUID；A 怪产生的弹幕或涌动顺序证据不能
  在切到 B 怪后释放触，切回 A 怪也不会复活已经清除的凭据
- 独立 45 秒小触只在上一成功弹幕/棱彩箭属于当前目标，并且 DurationObject 阈值正面
  证明奥术涌动剩余冷却严格大于 30 秒时放行；剩余时间在阈值内或无法证明时都保留触。
  大爆发配对仍要求涌动成功后 10 秒内接触，并同时作用于自有源和 JustAC 回退队列
