...
- 项目定制顺序覆盖：奥术涌动必须严格晚于一次新的大法师之触成功施放。只接受
  `UNIT_SPELLCAST_SUCCEEDED` 后 10 秒内、且晚于上次涌动的顺序证据；没有证据时核心
  会从自有源和 JustAC 回退队列中跳过涌动。触是目标 Debuff，不用光环猜测恢复凭据。
...
- 停步事件与试放碰撞时，当前试放实例被取消，下一安全技能至少导出一个完整刷新帧；
  再下一次计算若仍推荐飞弹，则它是新的静止动作实例，可以正常释放。
