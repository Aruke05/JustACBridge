...
- **保留爆发版**：仅当第一推荐属于保留项时，改用 JustAC 已经计算好的首个
  非保留、当前可用且已绑定动作；移动时队列无安全动作才使用同一套专精兜底。
...
12.1 再次改动循环：普通奥术飞弹应尽快排入下一技能、在 GCD 后主动截断；但带有
`Overpowered Missiles` `1277009` 的强化飞弹伤害翻倍、直接生成最大奥术齐射层数，
必须完整引导。12.1 策略因此同时登记 `clipChannels = { 5143 }` 和条件保护规则：
Bridge 在 `UNIT_SPELLCAST_SENT` 时（Buff 被本次飞弹消耗前）锁存强化状态，普通
飞弹开放 GCD 末续按，强化飞弹则整段屏蔽 M4/M5。这样 11.2、12.0、12.1 分别使用
对应的 Interface 版本策略，不再用单一无条件规则覆盖所有版本和 Proc 状态。
...
最终兜底同时是 M4/M5 的非空保证：正常队列选不出动作时，无论静止或移动，都会
直接导出当前专精登记的兜底快捷键。射程和运行时 `usable` 结果只写入诊断日志，
不再把最终兜底过滤成空动作；冰法因此始终落到冰枪术。
...
| 冰霜 | 冰霜之柱 `51271`、冰龙吐息基础/当前覆盖 `152279`/`1249658`、符文武器增效 `47568`、冰霜巨龙之怒 `279302`、死神印记 `439843`；12.1 追加亡者复生兼容 ID `46584`/`46585` |
...
12.1 天启骑士（Rider of the Apocalypse）按新版循环处理：冰霜巨龙之怒在冰霜之柱
开启后尽早释放以延长柱子并召集四骑士；召回尽量留给下一轮冰霜之柱。无损版继续
严格执行 JustAC 的资源/触发优先级，保留爆发版则把冰霜之柱、冰龙吐息、符文武器
增效、冰霜巨龙之怒及亡者复生作为同一套爆发资源留给玩家手动开启。
