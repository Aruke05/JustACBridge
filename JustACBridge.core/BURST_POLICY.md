...
12.1 奥法实战日志确认：自有源因光环层数 secret 回退时，JustAC 原队列会把作为通用
瞬发兜底的奥术弹幕排在奥术冲击之前，甚至因四项队列上限完全省略奥冲，导致静止
状态反复泄光奥术充能。两路只在 `fallback=true` 且原队列含未被证明的弹幕时，把
已知基础填充奥冲放到弹幕之前；若原队列没有奥冲则只补入该奥冲。静止时选择奥冲，
移动时核心过滤不可移动奥冲后仍选择弹幕。这不读取或猜测秘密光环，而是明确接受
可能错过少量最优弹幕窗口，以避免持续泄充能。自有源完整证明的弹幕分支返回独立
队列，不经过该保守处理。
