...
- 场地 10 秒到期只解除重复推荐过滤，不再发提醒；技能真实冷却或任一充能恢复时，
  默认显示两秒中央红字、播放 Raid Warning，并调用 WoW TTS 朗读“冷却就绪”；
- 充能会逐层监控，因此 `0/2→1/2` 与 `1/2→2/2` 都会各提醒一次；登录或重载时若
  已处于充能恢复中，也会接管剩余计时；
...
因此可以可靠监控主动饰品，纯被动/触发型饰品会被排除；登录、重载或换装时发现
