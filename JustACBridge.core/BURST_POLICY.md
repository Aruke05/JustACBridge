...
12.0 的奥术飞弹也不再全局强制在一个 GCD 后截断。当前规则恢复普通完整引导；
个别急速阈值和奥术之魂内的收益性截断继续交给 JustAC 当前推荐与玩家判断，不在
Bridge 中用一条无条件规则覆盖。这样 11.2 与 12.0 各自使用对应的 Interface
版本策略，不再把旧赛季结论套到当前客户端。
