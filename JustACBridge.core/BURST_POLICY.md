...
- **主推荐**：先处理策略明确登记且自身 Buff 确实缺失的维护技能，再于静止时逐帧
  透传当前推荐源的第一推荐；移动时跳过无法移动施放的读条、蓄力和引导，先从原
  队列选择首个安全动作；若前 8 项均不可执行，才使用当前专精登记的保守兜底动作。
...
错误方向发射或额外拉怪；M5 保持 JustAC 原推荐，由玩家手动面向后释放。
...
奥法棱彩屏障 `235450` 与冰法寒冰护体 `11426` 作为两路动作的护体维护项：只有
自身对应 Buff 被 API 明确报告为不存在、法术已学习、当前充能数明确大于 `1` 且
动作栏已有受支持快捷键时，才会优先于伤害队列推荐。自动循环只花额外充能，始终
保留 `1` 层供玩家手动应对机制；Buff 仍在或仅余一层时不自动释放，光环/充能状态
不可确认时也不猜测。攻略中“具体大伤前”的 encounter 预判仍不自动实现。
