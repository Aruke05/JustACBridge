...
```text
当前 Recommendation Source.GetQueue()
        ↓
JustACBridge 生成无损/保留爆发两个动作
        ↓
当前源或 JustAC 能力回退解析快捷键
        ↓
生成 72 字节协议包
        ↓
按位渲染为 48 列 × 12 行黑白矩阵
...
插件把矩阵锚定在 WoW 客户区左上角附近，名义偏移为 `(2, 7)`，名义单元大小
...
3. 从客户区坐标 `(2, 7)` 开始，捕获至少 `296 × 80` 物理像素。纵向下移
   `5 px`，避免顶边覆盖层遮挡传输矩阵。
...
| `0x20` | 保留爆发版动作已找到快捷键 |
| `0x40` | 玩家正在需要保护的持续引导（blocking channel） |
| `0x80` | 玩家正在普通读条或蓄力施法（casting/empowering） |

`0x40` 与 `0x80` 正常情况下互斥：

- 两者都是 `0`：空闲或瞬发技能；
- `0x40`：正在持续输出/引导，且职业策略未允许主动截断；
...
职业策略 `clipChannels` 可把循环明确要求主动截断的引导排除在 `0x40` 之外。
例如 11.2 奥术飞弹仍通过 SavedVariables/UI 报告为引导中，但 v3 的 GCD 门控只在
最后约 120 ms 开放下一动作，因此既不会整段占住动作队列，也不会过早截断。
12.0 与 12.1 奥术飞弹均完整引导并持续设置 `0x40`。虽然 12.1 攻略允许普通飞弹
截断，但在没有可靠实战证据证明能识别本次 `Overpowered Missiles` 前，不用推测
开放截断；只有 11.2 的明确旧版规则继续使用 `clipChannels`。12.1 浮光掠影允许
移动引导飞弹时仍完整保护本次引导，只是移动过滤不再错误跳过它。
...
| 位掩码 | 适用版本 | 含义 |
|---:|---:|---|
| `0x01` | v3+ | `queueReady`：GCD 空闲或进入发送窗口；2.13.1 起为 `min(120ms, 明确的 SpellQueueWindow)`，不可读时保留 120ms |
| `0x02` | v4 | 诊断包：玩家正在移动 |
| `0x04` | v4 | 诊断包：移动过滤已开启 |
...
- **主推荐**：静止时严格使用当前推荐源第一推荐；移动过滤开启时，使用推荐源
  队列中首个确认可移动施放的动作。基础瞬发、当前 Proc 瞬发和职业策略登记的
  移动施法例外会被保留；不可移动读条、蓄力和引导会被跳过。
- **射程安全**：默认跳过 JustAC 明确确认超出当前目标射程的动作；API 返回
  `nil`、secret 或无法判断时不擅自过滤。
- **距离顺序**：职业策略可在目标被明确判定超过指定距离时调整原队列内两个技能
  的先后。霜火冰超过 20 码时会把“冰川尖刺→冰风暴”改为先冰风暴；距离未知、
  未学霜火之箭或替代动作不可用/未绑定时保持 JustAC 原推荐。
- **场地生命周期**：DK 枯萎凋零/亵渎成功放置后记录固定到期时间；仍有效时
  跳过重复条目。场地状态通过 SavedVariables/API 导出，不占用 v3 像素包字段。
- **冷却就绪提醒**：枯萎凋零按服务器成功施放事件启动固定 30 秒顺序充能计时；
  饰品槽 1/2 的主动效果仍通过屏幕外 `Cooldown` 控件监听 `OnCooldownDone`。两者只
  显示游戏内文字/声音/TTS，不占用像素包字段。
- **保留爆发版**：始终按“持续按住”标准检查动作；即使第一推荐不是保留项，也只有
  在它可移动安全时才与无损版相同，否则继续扫描 JustAC 队列中第一个非保留、当前
  可用、已绑定且可移动安全的动作。不可移动读条、引导和蓄力不会因瞬时静止而放行。
- 依赖人物面向且无法可靠自动瞄准的技能可登记为保留版排除项；奥术宝珠由 M4 始终
  跳过，M5/M4 移动时及停止后的前 0.8 秒跳过，避免持续按住时向错误方向发射。
...
- 插件通过公共 GCD 法术 `61304` 计算剩余时间。GCD 剩余大于采用的发送窗口时
  `queueReady=0`，外部程序应吞掉功能键但不得发送动作；进入最后窗口或 GCD
  空闲时变为 `1`。
- 2.13.1 起上限保持 120ms，但尊重明确读到的更小 `SpellQueueWindow`（包括 0）；
  未知时保持原 120ms。窗口变化也递增 `sequence`，不改变动作相对优先级。
...
矩阵默认开启，锚定在客户区 `(2, 7)`，并且独立于普通两行 UI；隐藏普通面板
不会关闭像素接口。
